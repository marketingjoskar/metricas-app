...
function StatCard({ label, value, unit = '', icon, color, sub, delay = 0 }) {
  return (
    ...
    >
      ...
      >
        {value}
        <span style={{ fontSize: '1rem', marginLeft: 3 }}>{unit}</span>
      </div>
...
export default function SistemasDashboardPage() {
  ...
  const traficoTotal = ga4data
    ? (ga4data.trafico_organico || 0) +
      (ga4data.trafico_directo || 0) +
      (ga4data.trafico_social || 0) +
      (ga4data.trafico_referido || 0) +
      (ga4data.trafico_email || 0)
    : 0
      ...
      >
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.88rem' }}>
            Resumen mensual · Incidencias, imágenes y Google Analytics
          </p>
        </div>
        ...
        <>
          ...
          >
            <StatCard
              ...
              value={totals.incidencias}
              icon="🔧"
              color={color}
              delay={0}
              sub={`${totals.dias} día${totals.dias !== 1 ? 's' : ''} con registro`}
            />
            ...
            {ga4data && (
              <StatCard
                label="Sesiones web"
                value={ga4data.sesiones?.toLocaleString('es-AR') || '—'}
                icon="🌐"
                color="#10b981"
                delay={0.15}
              />
            ...
            {ga4data && (
              <StatCard
                ...
                value={ga4data.usuarios_activos?.toLocaleString('es-AR') || '—'}
                icon="👤"
                color="#8b5cf6"
                delay={0.2}
              />
          ...
          <div
            style={{
              display: 'grid',
              gridTemplateColumns: ga4data ? '1fr 1fr' : '1fr',
              gap: 14,
              marginBottom: 14,
            }}
          >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {[
                    ...
                    { label: 'Tasa de rebote', value: ga4data.tasa_rebote ? ga4data.tasa_rebote + '%' : null, color: '#f0436a' },
                    {
                      ...
                      value: ga4data.duracion_promedio_seg
                        ? Math.floor(ga4data.duracion_promedio_seg / 60) + 'm ' + Math.round(ga4data.duracion_promedio_seg % 60) + 's'
                        : null,
                      color: '#8b5cf6',
                    },
                  ].map((m) =>
          ...
          {ga4data?.seo_keywords?.length > 0 && (
            ...
            >
              ...
              <div style={{ padding: '14px 24px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                {[...ga4data.seo_keywords]
                  .sort((a, b) => parseInt(a.posicion || 99) - parseInt(b.posicion || 99))
                  .map((kw, i) => (
                    <div
                      key={i}
                      style={{
                    ...
                    >
                      ...
                      >
                        {kw.impresiones ? kw.impresiones + ' impr.' : '—'}
                      </span>
                    </div>
                  ))}
              </div>
                      ...
                      >
                        ...
                        <td
                          style={{
                            ...
                            textAlign: 'center',
                            fontFamily: 'var(--font-mono)',
                            color: r.incidencias_resueltas > 0 ? color : 'var(--text-muted)',
                            fontWeight: r.incidencias_resueltas > 0 ? 600 : 400,
                          }}
                        >
                          {r.incidencias_resueltas || 0}
                        </td>
                        ...
                        <td
                          style={{
                            ...
                            textAlign: 'center',
                            fontFamily: 'var(--font-mono)',
                            color: r.imagenes_codigos_actualizadas > 0 ? '#3b82f6' : 'var(--text-muted)',
                          }}
                        >
                          {r.imagenes_codigos_actualizadas || 0}
                        </td>
                        ...
                        <td
                          style={{
                            ...
                            textAlign: 'center',
                            fontFamily: 'var(--font-mono)',
                            color: r.imagenes_peso_optimizado > 0 ? '#0eb8d4' : 'var(--text-muted)',
                          }}
                        >
                          {r.imagenes_peso_optimizado || 0}
                        </td>
